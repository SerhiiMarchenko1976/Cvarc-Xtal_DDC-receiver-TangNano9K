...
// use 24 bits for the Accumulators. 
   ...
   assign weA 		= (x_avail && wstate == 0);
	...
	fir256 #(ABITS, TAPS) A (clock, waddr, weA, x_real, x_imag, RaccA, IaccA);
	fir256 #(ABITS, TAPS) B (clock, waddr, weB, x_real, x_imag, RaccB, IaccB);
	fir256 #(ABITS, TAPS) C (clock, waddr, weC, x_real, x_imag, RaccC, IaccC);
	fir256 #(ABITS, TAPS) D (clock, waddr, weD, x_real, x_imag, RaccD, IaccD);
	fir256 #(ABITS, TAPS) E (clock, waddr, weE, x_real, x_imag, RaccE, IaccE);
	fir256 #(ABITS, TAPS) F (clock, waddr, weF, x_real, x_imag, RaccF, IaccF);
	fir256 #(ABITS, TAPS) G (clock, waddr, weG, x_real, x_imag, RaccG, IaccG);
	fir256 #(ABITS, TAPS) H (clock, waddr, weH, x_real, x_imag, RaccH, IaccH);
...
module fir256(
    ...
    Gowin_pROM_256 rom(
        ...
        .ad(caddr) //input [7:0] ad
    );
	//ram16sdp
	//firram36 ram (clock, {x_real, x_imag}, raddr, waddr, we, q);  	// sample RAM 36 X 256;  36 bit == 18 bits I and 18 bits Q
	ram36x256 ram36(// sample RAM 36 X 256;  36 bit == 18 bits I and 18 bits Q
     .clock(clock),
     .data_in({x_real, x_imag}),
     .rdaddress(raddr),
     .wraddress(waddr),
     .wren(we), 
     .data_out(q)
	...
	begin
		if (we)		// Wait until a new sample is written to memory
			begin
				counter = TAPS[ADDRBITS:0] + 4'd4;			// count samples and pipeline latency (delay of 3 clocks from address being presented)
