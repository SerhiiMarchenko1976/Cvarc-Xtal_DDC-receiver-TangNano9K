  ...
  while(1){
    ...
    if (txrx_mode == RX_MODE) { //реакция на поворот энкодера на 0 экране
     if(show_volume > 1){
      ...
      if ((encoderDelta > 0) && (tun_mode == RIT || tun_mode == PAN)){pos_fft += 2;if (pos_fft > 450)pos_fft = 450;}
      if ((encoderDelta < 0) && (tun_mode == RIT || tun_mode == PAN)){pos_fft -= 2;if (pos_fft < 10)pos_fft = 10;}
      }
    }
