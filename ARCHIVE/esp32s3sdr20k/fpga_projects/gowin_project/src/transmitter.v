...
module Transmitter(
   ...
   // TX phase count
	localparam M2 = 32'd1501199875;  // B57 = 2^57.   M2 = B57/96MHz
   localparam M3 = 32'd16777216;   // M3 = 2^24, used to round the result
	wire [63:0] ratio = tx_freq * M2 + M3;
   wire [31:0] tx_tune_phase = ratio[56:25]; // 32'd251658240; // 7 MHz   
	...
	CicInterpM5 #(.RRRR(250), .IBITS(20), .OBITS(20), .GBITS(34))
        	in2 (clock, 1'd1, req2, y1_r, y1_i, y2_r, y2_i);

	// Tune transmitter with CORDIC
   wire signed [15:0]cordic_out_i;	
	cordic_tx #(.OUT_WIDTH(16))
 		cordic_inst (.clock(clock), .frequency(tx_tune_phase), .in_data_I(y2_i[19:4]),			
		.in_data_Q(y2_r[19:4]), .out_data_I(cordic_out_i), .out_data_Q());
