...
#define SPI_SCLK    (gpio_num_t)46
#define SPI_MISO    (gpio_num_t)5
#define SPI_MOSI    (gpio_num_t)9
#define SPI_SS      (gpio_num_t)14

//Encoder pins
#define ROTARY_ENCODER_A_PIN      (gpio_num_t)(43)
#define ROTARY_ENCODER_B_PIN      (gpio_num_t)(44)
...
#define START_FPGA     (gpio_num_t)(18)

//выход i2s аудио(на динамик через встроенный кодек)
#define STD_MCLK_OUT     (gpio_num_t)(-1)
#define STD_WS_OUT       (gpio_num_t)(2) 
#define STD_DIN_OUT      (gpio_num_t)(-1)   
#define STD_DOUT_OUT     (gpio_num_t)(41)
#define STD_BCLK_OUT     (gpio_num_t)(42)

//mems i2s-микрофон
#define STD_MCLK_IN_TX        (gpio_num_t)(-1)//
#define STD_WS_IN_TX          (gpio_num_t)(15)//(6)//
#define STD_DIN_IN_TX         (gpio_num_t)(6)//(7)//
#define STD_DOUT_IN_TX        (gpio_num_t)(-1)//
#define STD_BCLK_IN_TX        (gpio_num_t)(7)//(15)//
...
//LCD
#define LCD_BL  (gpio_num_t)1 //пин подсветки дисплея
...
int indent = 0;     
int bins = bandwidth/(i2s_sample_rate_rx/NUM_SAMPLE_BUF);
int bin_start = indent/(i2s_sample_rate_rx/NUM_SAMPLE_BUF);
int ptt;
...
uint8_t tun_mode =0;//0-tuning, 1-rit,2-PAN
...
uint32_t rx_freq = freq+hz*pos_fft; //частота приема
uint32_t rit_freq = freq+hz*pos_fft;//частота расстройки приема
uint32_t tx_freq = freq+hz*pos_fft; //частота передачи
