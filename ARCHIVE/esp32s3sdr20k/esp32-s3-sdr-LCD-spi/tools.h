...
void action() { //обработка нажатий на кнопки
  if (txrx_mode == RX_MODE) { //на экране приема
    ...
    if (lkey == 7) {tun_mode++;if (tun_mode > PAN)tun_mode = TUN;if (tun_mode == TUN)pos_fft = PCH;}
    if (lkey == 6) {tp_control=false;change_rx_rate = true;show_srate = SHOW_VALUE; show_band=-1;show_conf=-1;show_volume=-1;}
    if (lkey == 5) {tp_control=false;if (rf_mode > AM)rf_mode = LSB;}
    if (lkey == 9) {agc_coeff+=1;if(agc_coeff>100)agc_coeff=100;show_volume=SHOW_VALUE;show_band=-1;show_srate=-1;show_conf=-1;}
    if (lkey ==10) {agc_coeff-=1;if(agc_coeff>101)agc_coeff=0;show_volume=SHOW_VALUE;show_band=-1;show_srate=-1;show_conf=-1;}
    if (lkey ==11) {
      flag_write_parameters = true; //кнопка энкодера(сохранение текущих парметров)
      show_conf = SHOW_VALUE;show_band=-1;show_srate=-1; show_volume=-1;
    }
    lkey = 0;bkey=0;
  }
  ...
  while(1){
    ...
    if (txrx_mode == RX_MODE) { //реакция на поворот энкодера на 0 экране
     if(show_volume > 1){
      ...
      if ((encoderDelta > 0) && (tun_mode == TUN)){freq = freq + step_freq[numstep];if (freq > 39999999)freq = 39999999;bands[numband].freq=freq;}
      if ((encoderDelta < 0) && (tun_mode == TUN)){freq = freq - step_freq[numstep];if (freq < 100000)freq = 100000;bands[numband].freq=freq;}
      if ((encoderDelta > 0) && (tun_mode == RIT || tun_mode == PAN)){pos_fft += 2;if (pos_fft > 500)pos_fft = 500;}
      if ((encoderDelta < 0) && (tun_mode == RIT || tun_mode == PAN)){pos_fft -= 2;if (pos_fft < 4)pos_fft = 4;}
      }
    }
...
#define BTN_FREQ_PLUS  (tp_x>280&&tp_x<470&&tp_y>80&&tp_y<150&&!tp_control&&!tp_band)
#define BTN_FREQ_MINUS (tp_x>25&&tp_x<220&&tp_y>70&&tp_y<150&&!tp_control&&!tp_band)
#define BTN_SAVE_CONF  (tp_x>30&&tp_x<220&&tp_y>20&&tp_y<65)
#define BTN_VOL_PLUS   (tp_x>270&&tp_x<470&&tp_y>200&&tp_y<270&&!tp_control&&!tp_band)
#define BTN_VOL_MINUS  (tp_x>25&&tp_x<220&&tp_y>200&&tp_y<270&&!tp_control&&!tp_band)
...
void control() {
  ...
  tx_tune_phase=calc_tune_phase(tx_freq,122880000);
  check_ptt();
  if (txrx_mode == RX_MODE) {
    if (tun_mode == RIT)tx_freq = freq + PCH * hz; else tx_freq = freq + pos_fft * hz;
    rit_freq = freq + pos_fft * hz;
    rx_freq = rit_freq;
    change_band();
