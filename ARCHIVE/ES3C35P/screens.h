...
void draw_spectr(){//спектр
      ...
      int static p_x=0;
      int16_t col_s;
      int width_b = bandwidth/(i2s_sample_rate_rx/NUM_SAMPLE_BUF)/2;
      int indent_b = indent/(i2s_sample_rate_rx/NUM_SAMPLE_BUF)/2;
...
void draw_service(){
    ...
    if (numstep==4) step = "1000";
        ...
        gfx.setFreeFont(&CourierCyr6pt8b);gfx.setTextColor(WHITE,BLACK);
        ...
        gfx.print("RIT ");gfx.printf("%2d %03d,%03d",  rit_freq/1000000, (rit_freq/1000)%1000, (rit_freq/10)%100 );
        ...
        gfx.print("XIT ");gfx.printf("%2d %03d,%03d",  tx_freq/1000000, (tx_freq/1000)%1000, (tx_freq/10)%100 );
        
      int c_freq = freq+hz*PCH*2; //
      gfx.setFreeFont(&Seven_Segment18pt8b);gfx.setTextSize(1);
      ...
      for(int i=0;i<NUM_SAMPLE_BUF/2;i+=52){ //шкала видимых частот
        ...
        c_freq=freq+2*hz*i;
        gfx.printf("%2d%03d",  c_freq/1000000, (c_freq/1000)%1000, (c_freq/10)%100 );
...
void draw_smeter(){
  ...
  static int d_x=x;
  static int d_y=y;
  gfx.pushImage(35+xwin,1+ywin,184,60,(uint16_t*)&s_smeter);
  float _smeter = -PI*(150-old_smeter)/205;
  d_x=x+cos(_smeter)*h;
  d_y= y+sin(_smeter)*h;
  gfx.drawLine(x, y, d_x, d_y,GREEN);
  gfx.fillRect(50+xwin,60+ywin,180,9,BLACK);
  //if(wifi)gfx.pushImage(3,5,15,15,(uint16_t*)&wifi_on);
  //else gfx.pushImage(3,5,15,15,(uint16_t*)&wifi_off);
}
...
void draw_temp_value(){
  ...
  if(show_band>1){
    ...
    gfx.setFreeFont(&Seven_Segment18pt8b);
    gfx.setTextSize(2);
    ...
    gfx.setFreeFont(&Seven_Segment18pt8b);
    gfx.setTextSize(2);
    gfx.setCursor(12+xwin,150+ywin);
    if(tun_mode==0){gfx.print("Tun_mode: TUN");}
    if(tun_mode==1){gfx.print("Tun_mode: RIT");}
    if(tun_mode==2){gfx.print("Tun_mode: PAN");}
    //gfx.print(agc_coeff); // tuning, 1-rit,2-PAN
    //gfx.print(bands[numband].name);
    //show_tun_mode = -1;
    gfx.setTextSize(1);
    return;
...
void drawinfo(){
      gfx.setFreeFont(&FreeMonoBold8pt8b);gfx.setTextColor(WHITE);
      
      //gfx.setCursor(5,250);
      //gfx.print(tp_x);gfx.print(":");gfx.print(tp_y);gfx.print(":");gfx.print(tp_size);
      
      if(draw_info){
        gfx.fillRect(xwin,272+ywin,480,50,BLACK);
        if(t_fps<=0)t_fps=1;
        gfx.setCursor(420,310);
...
void x_display(){
   ...
   draw_Grid(0+xwin,72+ywin,479,103);//нарисовать сетку на спектре
   draw_temp_value();
